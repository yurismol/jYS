...
mSNPClass <- R6::R6Class(
    ...
    private = list(
        ...
        .init = function() {
            ...
            assocTable$setNote('n3', .("<b>Genetic models</b> test the association under different inheritance patterns: <b>Codominant</b> (AA vs AB vs BB), <b>Dominant</b> (AA vs AB+BB), <b>Recessive</b> (AA+AB vs BB), <b>Overdominant</b> (AA+BB vs AB), <b>Log-additive</b> (trend test for allele dose 0/1/2)."))
            assocTable$setNote('n4', .("<b>p-adjusted</b> is the p-value corrected for multiple testing across markers using the selected method (Bonferroni, Holm, or Benjamini–Hochberg FDR)."))
            assocTable$setNote('n5', .("<b>Best model selection</b>: the genetic model with the lowest p-value (or AIC/BIC) is highlighted. This helps identify the most likely mode of inheritance for the observed association."))
            
            popTable$setNote('n1', .("<b>MAF (Minor Allele Frequency)</b> is the frequency of the less common allele in the population. Markers with <b>MAF < 0.01</b> are typically considered monomorphic and may be excluded from analysis. <b>MAF < 0.05</b> indicates a rare variant."))
            popTable$setNote('n2', .("<b>PIC (Polymorphism Information Content)</b> measures the informativeness of a genetic marker. <b>PIC > 0.5</b> is highly informative, <b>0.25 < PIC < 0.5</b> is moderately informative, <b>PIC < 0.25</b> is slightly informative (Botstein et al., 1980)."))
            popTable$setNote('n3', .("<b>Shannon Information Index (I)</b> quantifies allelic diversity. Higher values indicate greater diversity. For a biallelic marker, maximum I = ln(2) ≈ 0.693 when allele frequencies are equal."))
            popTable$setNote('n4', .("<b>Effective number of alleles (Ne)</b> is the reciprocal of homozygosity: Ne = 1 / Σ(pi²). Ne = 1 indicates fixation (monomorphic), Ne = 2 indicates equal allele frequencies for a biallelic marker."))
            
            mdrBestTable$setNote('n1', .("<b>MDR (Multifactor Dimensionality Reduction)</b> (Ritchie et al., 2001) is a non-parametric method for detecting gene-gene interactions (epistasis). It reduces high-dimensional genotype combinations into a binary variable (high-risk vs. low-risk)."))
            mdrBestTable$setNote('n2', .("<b>Balanced Accuracy (BA)</b> = (Sensitivity + Specificity) / 2. Accounts for class imbalance. BA = 0.5 indicates random classification."))
            mdrBestTable$setNote('n3', .("<b>Cross-Validation Consistency (CVC)</b> is the number of folds (out of K) in which the model was selected as best. Higher CVC indicates greater stability of the identified interaction."))
            mdrBestTable$setNote('n4', .("<b>Training BA</b> may be optimistically biased. <b>Testing BA</b> is computed on the held-out fold and provides an unbiased estimate of predictive performance."))
            mdrBestTable$setNote('n5', .("<b>Permutation p-value</b> is obtained by shuffling outcome labels. It represents the proportion of permuted datasets that achieved a testing BA ≥ the observed value."))
            
            mdrCellTable$setNote('n1', .("<b>High-Risk (HR)</b> cells have cases/controls ratio ≥ threshold T. <b>Low-Risk (LR)</b> cells have ratio < T."))
            mdrCellTable$setNote('n2', .("<b>Ratio</b> = Cases / Controls. Empty cells (no observations) are left unclassified."))
            mdrCellTable$setNote('n3', .("<b>Classification</b> defines the epistatic model: subjects in HR cells are predicted as cases, LR cells as controls."))
            
            ldTable$setNote('n1', .("<b>D' (normalized LD)</b> measures the strength of linkage disequilibrium between two loci, scaled to [-1, 1]. <b>|D'| = 1</b> indicates complete LD (no recombination observed), <b>D' = 0</b> indicates linkage equilibrium (independent assortment)."))
            ldTable$setNote('n2', .("<b>r² (squared correlation)</b> measures both LD strength and allele frequency similarity. <b>r² = 1</b> indicates perfect LD (complete mutual predictability), <b>r² > 0.8</b> is commonly used as the threshold for strong LD in GWAS tag-SNP selection. r² is preferred over D' for association study design because it directly relates to statistical power."))
            ldTable$setNote('n3', .("<b>Haplotype frequencies</b> are estimated using the Expectation-Maximization (EM) algorithm from unphased genotype data. The p-value tests the null hypothesis D = 0 (linkage equilibrium)."))
        },
